...
let resetCooldownInterval; // Global variable for the reset countdown
const RESET_DELAY = 300; // Constant for the reset delay in seconds (5 minutes = 300 seconds)
const AD_DURATION = 15; // seconds
const DAILY_AD_LIMIT = 20; //
const COOLDOWN_DURATION = 5; // seconds
const BEEP_SOUND_URL = 'sounds/beep.wav'; //
...
const statusMessageDiv = document.getElementById('statusMessage'); // Reused for reset warning
...
const LS_RESET_START_TIMESTAMP = 'resetStartTimestamp'; // NEW: Key to store reset start time
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Ensure status message is hidden on load unless explicitly set
    if (statusMessageDiv.textContent === "") { //
        statusMessageDiv.style.display = 'none'; //
    }

    // NEW: Check for pending reset on app load
    checkPendingReset(); //
});
...
function checkDailyLimitAndCooldown() {
    ...
    // NEW: Don't allow viewing ads if a reset is pending
    if (localStorage.getItem(LS_RESET_START_TIMESTAMP)) { //
        viewAdBtn.disabled = true; //
        return; //
    }
    ...
    if (dailyCount >= DAILY_AD_LIMIT) { //
        ...
        statusMessageDiv.style.display = 'none'; // Ensure this is hidden when daily limit reached
...
function handleViewAd() {
    ...
    // Check for VPN before starting the ad
    checkVpnStatus().then(vpnActive => { // NEW: Asynchronously check VPN status
        if (vpnActive) {
            isVpnDetected = true;
            viewAdBtn.disabled = true;
            closeAppBtn.disabled = false;
            resetAllDataBtn.disabled = false;
            showPopup(vpnWarningPopup, 0); // Show warning indefinitely
            vpnWarningPopup.querySelector('.popup-content').textContent = "VPN/Proxy Detected! Please disable it to view ads.";
            adDisplay.innerHTML = `<p>VPN/Proxy Detected! Ad cannot be viewed successfully. Please disable VPN and try again.</p>`;
            timerContainer.style.display = 'none';
            statusMessageDiv.style.display = 'none';
            console.warn("VPN/Proxy detected before ad starts. Ad viewing halted.");
            return; // Stop here if VPN is active
        }

        // If no VPN, proceed with ad loading
        isAdPlaying = true; //
        isVpnDetected = false; //
        ...
        resetAllDataBtn.disabled = true; // Disable reset button during ad view
        ...
        statusMessageDiv.style.display = 'none'; // Hide other messages
        ...
        // A slight delay before checking ad content, just in case the iframe needs to load
        setTimeout(() => {
            checkAdContentForVPN(); //
        }, 2000);
    }).catch(error => {
        console.error("Error checking VPN status:", error);
        // Fallback or error handling if VPN check fails
        viewAdBtn.disabled = true;
        showPopup(vpnWarningPopup, 3000);
        vpnWarningPopup.querySelector('.popup-content').textContent = "Could not check VPN status. Please try again.";
    });
}

// NEW: Function to check for VPN/Proxy using a public service
async function checkVpnStatus() {
    try {
        // Using a free API to check IP for VPN/proxy status
        // Note: Free APIs might have rate limits or not be 100% accurate
        const response = await fetch('https://vpnapi.io/api/?key=YOUR_VPN_API_KEY'); // Replace with your actual VPN API Key
        const data = await response.json();
        
        // Check if the API response indicates a VPN or proxy
        return data.security && (data.security.vpn || data.security.proxy);
    } catch (error) {
        console.error("Error fetching VPN status:", error);
        return false; // Assume no VPN if check fails (or handle as an error)
    }
}
...
function stopTimer() {
    ...
    isAdPlaying = false; //
    closeAppBtn.disabled = false; //
    resetAllDataBtn.disabled = false; // Re-enable reset button after ad stops
    timerContainer.style.display = 'none'; //
    if (!isVpnDetected) { //
        checkDailyLimitAndCooldown(); //
    }
...
function checkAdContentForVPN() {
    // This check is very basic and relies on specific text from the ad.
    // A more robust VPN detection is typically done server-side or with more advanced client-side scripts.
    const adIframe = document.getElementById('adIframe'); //
    if (!adIframe || !adIframe.contentWindow) return; //

    try {
        const iframeDoc = adIframe.contentWindow.document; //
        const adContent = iframeDoc.body.textContent || iframeDoc.body.innerText; //
        const proxyDetectedMessage = "Anonymous Proxy detected."; // Example message from ad network
        ...
        if (adContent.includes(proxyDetectedMessage)) { //
            ...
            viewAdBtn.disabled = true; //
            closeAppBtn.disabled = false; //
            resetAllDataBtn.disabled = false; // Re-enable reset button
            showPopup(vpnWarningPopup, 0); //
            vpnWarningPopup.querySelector('.popup-content').textContent = "VPN/Proxy Detected! Ad cannot be viewed successfully. Please disable VPN."; //
        ...
        // This catch block handles cross-origin iframe access errors.
        // If the iframe content cannot be accessed, we cannot perform the client-side VPN check.
        // In a real-world scenario, you'd rely on server-side checks or an ad network's own reporting.
        console.warn("Could not access iframe content for VPN check due to cross-origin policy:", e); //
        // We assume no VPN detected if we can't check
        isVpnDetected = false; //
    }
}
...
function adViewedSuccessfully() {
    if (!isVpnDetected) { //
        ...
        // --- NEW: Send data to Telegram Bot ---
        if (window.Telegram && window.Telegram.WebApp && window.Telegram.WebApp.sendData) { //
            const userId = userIdSpan.textContent; // පරිශීලක ID ලබා ගැනීම
            const currentTotalAds = parseInt(localStorage.getItem(LS_TOTAL_ADS_VIEWED)) || 0; // වර්තමාන මුළු දැන්වීම් ගණන

            // බොට් එකට යැවීමට අවශ්‍ය දත්ත (JSON format)
            const dataToSend = { //
                type: 'ad_viewed', // දත්ත වර්ගය හඳුනා ගැනීමට
                userId: userId, //
                totalAds: currentTotalAds, // හෝ increment වූ ගණන පමණක් (උදා: 1)
                timestamp: Date.now() //
            };
        ...
        showPopup(vpnWarningPopup, 0); //
        vpnWarningPopup.querySelector('.popup-content').textContent = "Ad viewing failed due to VPN/Proxy. Please disable VPN."; //
...
function initiatePersistentReset() {
    // Disable buttons to prevent interference
    resetAllDataBtn.disabled = true; //
    viewAdBtn.disabled = true; //
    closeAppBtn.disabled = true; //

    const resetStartTimestamp = parseInt(localStorage.getItem(LS_RESET_START_TIMESTAMP)); //
    const currentTime = Date.now(); //
    let timeElapsed = 0; //

    if (resetStartTimestamp) { //
        timeElapsed = (currentTime - resetStartTimestamp) / 1000; // time elapsed in seconds
    } else {
        // If no timestamp, this is a new reset request
        localStorage.setItem(LS_RESET_START_TIMESTAMP, currentTime); //
    }

    let remainingResetTime = Math.max(0, RESET_DELAY - timeElapsed); //

    // If time has already passed, perform reset immediately
    if (remainingResetTime <= 0) { //
        performReset(); //
        return; //
    }

    // Show a temporary warning message with countdown
    statusMessageDiv.classList.add('reset-warning'); //
    statusMessageDiv.style.display = 'block'; //
    statusMessageDiv.textContent = `WARNING: All data will be reset in ${formatTime(remainingResetTime)}! Do not close the app.`; //
    
    clearInterval(resetCooldownInterval); // Clear any old interval
    resetCooldownInterval = setInterval(() => { //
        remainingResetTime--; //
        statusMessageDiv.textContent = `WARNING: All data will be reset in ${formatTime(remainingResetTime)}! Do not close the app.`; //

        if (remainingResetTime <= 0) { //
            clearInterval(resetCooldownInterval); // Stop the countdown
            performReset(); //
        }
...
function performReset() {
    ...
    localStorage.removeItem(LS_RESET_START_TIMESTAMP); // NEW: Clear the reset timestamp
    ...
    statusMessageDiv.textContent = "All data has been reset successfully!"; //
    statusMessageDiv.classList.remove('reset-warning'); //
    statusMessageDiv.classList.add('success-message'); //

    setTimeout(() => { //
        statusMessageDiv.style.display = 'none'; // Hide success message after a short delay
    ...
    viewAdBtn.disabled = false; //
    closeAppBtn.disabled = false; //
    checkDailyLimitAndCooldown(); // Re-evaluate view ad button status
}


function handleResetAllData() {
    // Prevent starting a new countdown if one is already active
    if (localStorage.getItem(LS_RESET_START_TIMESTAMP)) { //
        alert("A data reset is already in progress. Please wait for it to complete."); //
        initiatePersistentReset(); // Re-show countdown if app was closed
        return; //
    }

    if (confirm("Are you sure you want to reset all data? This action cannot be undone and will begin in " + (RESET_DELAY / 60) + " minutes. Do not close the app during the countdown!")) { //
        initiatePersistentReset(); //
    }
}

// NEW: Function to check for a pending reset when the app starts
function checkPendingReset() {
    const resetStartTimestamp = localStorage.getItem(LS_RESET_START_TIMESTAMP); //
    if (resetStartTimestamp) { //
        const currentTime = Date.now(); //
        const timeElapsed = (currentTime - parseInt(resetStartTimestamp)) / 1000; //
        
        if (timeElapsed >= RESET_DELAY) { //
            // Time has passed, perform the reset immediately
            performReset(); //
        } else {
            // Time has not passed, resume the countdown
            initiatePersistentReset(); // This will recalculate remaining time and start countdown
        }
    }
}

// Helper function to format time into minutes and seconds (e.g., "05:00")
function formatTime(seconds) {
    const minutes = Math.floor(seconds / 60); //
    const remainingSeconds = seconds % 60; //
    const paddedSeconds = remainingSeconds < 10 ? '0' + remainingSeconds : remainingSeconds; //
    // Ensure minutes also have leading zero if less than 10
