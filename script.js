...
// Reset delay duration (3 minutes)
...
let resetDelayInterval; // Global variable for the reset countdown interval
...
// DOM elements for reset countdown
...
const LS_RESET_COUNTDOWN_END_TIMESTAMP = 'resetCountdownEndTimestamp';
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    checkOngoingResetCountdown();

    // Check for VPN/Proxy on app load
    const isVpn = await checkVpnProxy();
    if (isVpn) {
        // Show VPN warning for 6 seconds on load and close app
        showPopupAndCloseApp(vpnWarningPopup, 6000);
    }
...
function handleViewAd() {
    ...
    checkVpnProxy().then(isVpn => {
        isVpnDetected = isVpn;
        if (isVpnDetected) {
            // Show VPN warning for 6 seconds and close app
            showPopupAndCloseApp(vpnWarningPopup, 6000);
            return;
        }
        ...
        if (timeSinceLastAd < COOLDOWN_DURATION) {
            updateCooldownMessage(); // Ensure cooldown message is updated
            return;
        }
...
async function checkVpnProxy() {
    try {
        ...
        // NOTE: Replace 'YOUR_VPN_API_KEY' with an actual API key from a service like ip-api.com, ipdata.co, etc.
        const vpnCheckResponse = await fetch(`https://vpnapi.io/api/${ipAddress}?key=YOUR_VPN_API_KEY`);
...
// Function to show a popup and then close the app
function showPopupAndCloseApp(popupElement, duration, message = null) {
    // First, show the popup using the existing showPopup function
    showPopup(popupElement, duration, message); // This will handle showing and then hiding the popup

    // Then, schedule the app closure after the specified duration
    setTimeout(() => {
        handleCloseApp();
    }, duration);
}
...
// Checks if there's an ongoing reset countdown from a previous session
function checkOngoingResetCountdown() {
    const resetEndTime = parseInt(localStorage.getItem(LS_RESET_COUNTDOWN_END_TIMESTAMP)) || 0;
    const now = Date.now();

    if (resetEndTime > now) {
        // Reset countdown was active and still ongoing
        let remainingTime = Math.ceil((resetEndTime - now) / 1000); // Remaining seconds

        resetAllDataBtn.disabled = true; // Disable button
        resetCountdownMessageDiv.style.display = 'block';
        resetCountdownTimerSpan.textContent = formatTime(remainingTime);
        ...
        resetDelayInterval = setInterval(() => {
            ...
            if (remainingTime <= 0) {
                clearInterval(resetDelayInterval);
                localStorage.removeItem(LS_RESET_COUNTDOWN_END_TIMESTAMP); // Clear timestamp
    ...
    } else if (resetEndTime > 0 && resetEndTime <= now) {
        // Countdown ended while app was closed or just after opening
        localStorage.removeItem(LS_RESET_COUNTDOWN_END_TIMESTAMP); // Clear timestamp
        performResetLogic(); // Perform the actual reset
        showPopup(successPopup, 5000, "All data has been reset successfully!");
        resetAllDataBtn.disabled = false; // Re-enable button
    }
    // If resetEndTime is 0, no ongoing reset, do nothing.
}
...
function handleResetAllData() {
    if (confirm("Are you sure you want to reset all data? This action cannot be undone.")) {
        ...
        // Save the end timestamp of the reset countdown to localStorage
        localStorage.setItem(LS_RESET_COUNTDOWN_END_TIMESTAMP, Date.now() + RESET_DELAY_DURATION * 1000);
